...
public class OperuitMain {
    ...
    public void run() throws IOException {
        ...
        databaseService.execute(() -> "CREATE TABLE IF NOT EXISTS `users` (`username` VARCHAR(64), `display_name` VARCHAR(128), `password` VARCHAR(4096), `birthday` VARCHAR(24), `gender` VARCHAR(24), `private_key` TEXT, PRIMARY KEY `username`)").thenAccept(queryResult -> {});
    }
